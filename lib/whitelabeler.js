...
function updateMauticPrimary(color) {
  ...
  $('body').append(
      '<style id="primary">.sidebar a.btn-support { background-color:' +
      mautic_primary + ' !important; border-color: ' + mautic_primary +
      ' !important; }</style>');

  $('li.support').css({
    'box-shadow' : '0 0 30px inset ' + mautic_primary,
    'border-color' : mautic_primary
  });
...
function updateMauticHover(color) {
  ...
  $('body').append(
      '<style id="button-hover">button.btn-default:hover, button#login_button:hover, li.support a.btn-support:hover { background-color:' +
      mautic_hover + ' !important; border-color: ' + mautic_hover +
      ' !important; }</style>');
...
function updateSidebarLinkHover(color) {
  ...
  $('style#hover').remove();
  $('body').append('<style id="hover">div.sidebar a:hover { color:' +
                   sidebar_link_hover + ' !important; }</style>');
...
function updateSidebarDivider(color) {
  ...
  $('style#border').remove();
  $('body').append(
      '<style id="border">.sidebar > li > a::after { border-bottom: 1px solid ' +
      sidebar_divider +
      ' !important; } .nav-submenu::after { background-color:' +
      sidebar_divider + ' !important }</style>');
...
function updateSubmenuBulletBg(color) {
  ...
  $('body').append(
      '<style id="bullet-bg">.sidebar li .nav-submenu .nav-group::after { background-color: ' +
      submenu_bullet_bg + ' !important; }</style>');
...
function updateSubmenuBulletShadow(color) {
  ...
  $('body').append(
      '<style id="bullet-shadow">.sidebar li .nav-submenu .nav-group::after { box-shadow: 0 0 0 2px ' +
      submenu_bullet_shadow + ' !important; }</style>');
...
function updateDivderLeft(left) {
  ...
  $('body').append('<style id="border-left">.sidebar > li > a::after { left: ' +
                   divider_left + 'px !important; }</style>');
}
...
function updateLoginLogoWidth(width) {
  ...
  $('#login_preview img#login-logo, div.mautic-logo')
      .css('width', login_logo_width + 'px');
}
...
function loadJsonConfig(config_file = 'config.json') {
  ...
  $.get(window.location.href, {q : 'saved', file : config_file}, function(data) {
    ...
    if (data.status == 1) {
      ...
      if (data.data.submenu_bullet_shadow &&
          data.data.submenu_bullet_shadow != '') {
      ...
      if (data.data.sidebar_logo_margin_top &&
          data.data.sidebar_logo_margin_top != '') {
        updateSidebarLogoMarginTop(data.data.sidebar_logo_margin_top);
      }

      // Sidebar Logo Margin Left
      if (data.data.sidebar_logo_margin_left &&
          data.data.sidebar_logo_margin_left != '') {
        updateSidebarLogoMarginLeft(data.data.sidebar_logo_margin_left);
      }

      // Sidebar Logo Margin Right
      if (data.data.sidebar_logo_margin_right &&
          data.data.sidebar_logo_margin_right != '') {
      ...
      if (data.data.login_logo_margin_top &&
          data.data.login_logo_margin_top != '') {
        updateLoginLogoMarginTop(data.data.login_logo_margin_top);
      }

      // Login Logo Margin Bottom
      if (data.data.login_logo_margin_bottom &&
          data.data.login_logo_margin_bottom != '') {
      ...
      if (data.data.sidebar_logo) {
        ...
        $.get(window.location.href, {
           q : 'asset',
           url :
               encodeURIComponent(wl_url + '/assets/' + data.data.sidebar_logo)
         }).done(function(d) {
          if (d == 1) {
            $('img#mautic-logo')
                .attr('src', 'assets/' + data.data.sidebar_logo);
            $('div#sidebar-logo-upload').hide();
            $('div#sidebar-logo-upload').val(null);
            $('div#sidebar-logo-loaded span')
                .text('assets/' + data.data.sidebar_logo);
            ...
            $('span.sidebar-logo-error')
                .text(
                    data.data.sidebar_logo +
                    ' is defined in config.json, but was not found in the assets folder.')
                .fadeIn();
          }
        });
      }
      ...
      if (data.data.login_logo) {
        ...
        $.get(window.location.href, {
           q : 'asset',
           url : encodeURIComponent(wl_url + '/assets/' + data.data.login_logo)
         }).done(function(d) {
          if (d == 1) {
            $('img#login-logo').attr('src', 'assets/' + data.data.login_logo);
            $('div#login-logo-upload').hide();
            $('div#login-logo-loaded span')
                .text('assets/' + data.data.login_logo);
            ...
            $('span.login-logo-error')
                .text(
                    data.data.login_logo +
                    ' is defined in config.json, but was not found in the assets folder.')
                .fadeIn();
          }
        });
      ...
      if (data.data.favicon) {
        ...
        $.get(window.location.href, {
           q : 'asset',
           url : encodeURIComponent(wl_url + '/assets/' + data.data.favicon)
         }).done(function(d) {
          if (d == 1) {
...
// mautic_primary
$("input#mautic-primary")
    .on('move.spectrum change.spectrum',
        function(e,
                 tinycolor) { updateMauticPrimary(tinycolor.toHexString()); });

// mautic_hover
$("input#mautic-hover")
    .on('move.spectrum change.spectrum',
        function(e, tinycolor) { updateMauticHover(tinycolor.toHexString()); });

// logo_bg
$("input#logo-bg")
    .on('move.spectrum change.spectrum',
        function(e, tinycolor) { updateLogoBg(tinycolor.toHexString()); });

// sidebar_bg
$("input#sidebar-bg")
    .on('move.spectrum change.spectrum',
        function(e, tinycolor) { updateSidebarBg(tinycolor.toHexString()); });

// sidebar_submenu_bg
$("input#sidebar-submenu-bg")
    .on('move.spectrum change.spectrum',
        function(
            e, tinycolor) { updateSidebarSubmenuBg(tinycolor.toHexString()); });

// sidebar_link
$("input#sidebar-link")
    .on('move.spectrum change.spectrum',
        function(e, tinycolor) { updateSidebarLink(tinycolor.toHexString()); });

// sidebar_link_hover
$("input#sidebar-link-hover")
    .on('move.spectrum change.spectrum',
        function(
            e, tinycolor) { updateSidebarLinkHover(tinycolor.toHexString()); });

// active_icon
$("input#active-icon")
    .on('move.spectrum change.spectrum',
        function(e, tinycolor) { updateActiveIcon(tinycolor.toHexString()); });

// sidebar_divider
$("input#sidebar-divider")
    .on('move.spectrum change.spectrum',
        function(e,
                 tinycolor) { updateSidebarDivider(tinycolor.toHexString()); });

// submenu_bullet_bg
$("input#submenu-bullet-bg")
    .on('move.spectrum change.spectrum',
        function(
            e, tinycolor) { updateSubmenuBulletBg(tinycolor.toHexString()); });

// submenu_bullet_shadow
$("input#submenu-bullet-shadow")
    .on('move.spectrum change.spectrum',
        function(
            e,
            tinycolor) { updateSubmenuBulletShadow(tinycolor.toHexString()); });
...
$('body').on('change', 'input#mautic-path', function() { updatePath(); });
...
function updateUrl() {
  ...
  $.get(window.location.href, {
     q : 'url',
     url : encodeURIComponent($('input#mautic-url').val())
   }).done(function(data) {
    console.log(data);
    ...
    if (data.status == 1) {
      ...
      $($('input#mautic-url'))
          .parent('div')
          .parent('div')
          .children('small.url-fail')
          .text(data.message)
          .fadeIn(function() { $('div#overlay').fadeOut(); });
    }
  });
}
$('body').on('change', 'input#mautic-url', function() { updateUrl(); });
...
|--------------------------------------------------------------------------
*/
$('body').on('keyup', 'input#company-name, input#footer-prefix, input#footer',
             function() { updateFooterPreview(); });
...
$("input#sidebar-logo-file, input#login-logo-file, input#favicon-file")
    .change(function() {
      readImg(this);
...
$('body').on(
    'keyup change', 'input#sidebar-logo-width',
    function() { $('img#mautic-logo').css('width', $(this).val() + 'px'); });
...
$('body').on('keyup change', 'input#divider-left',
             function() { updateDivderLeft($(this).val()); });
...
function saveConfigJson(file = false) {
  ...
  if ($('#favicon-file')[0].files.length == 1) {
    ...
    if ($('#favicon-file')[0].files[0].name.split('.')[1].toLowerCase() ==
        'ico') {
    ...
    if (favicon) {
      ...
      if ($('#login-logo-file')[0].files.length == 1) {
        favicon = 'favicon-' +
                  $('#login-logo-file')[0].files[0].name.split('.')[0] + '.ico';
      } else {
  ...
  if ($('#sidebar-logo-file')[0].files.length > 0 ||
      $('#login-logo-file')[0].files.length > 0 ||
      $('#favicon-file')[0].files.length > 0) {
      ...
      success : function(result) {
        if (result.status == 1) {
          ...
          if (file != false) {
            ...
            $.post(
                window.location.href + '?q=save-as',
                {'file' : file, 'config' : config}, function(data) {
                  if (data) {
                    ...
                    console.log(
                        'There was an error saving your data. Please review your server error log.')
                  }
                });
            ...
            $.post(
                window.location.href + '?q=save', {'config' : config},
                function(data) {
                  if (data) {
                    $('button i.save-loading').hide();
                    $('button i.fa-floppy-o').show();
    ...
    if (file != false) {
      ...
      $.post(window.location.href + '?q=save', {'config' : config}, function(data) {
        if (data) {
          $('button i.save-loading').hide();
          $('button i.fa-floppy-o').show();
...
$('body').on('click', 'button#save-as', function(e) {
  e.preventDefault();
  var filename = prompt(
      'Give this configuration a name (don\'t include the .json extension):');
  if (filename) {
...
function clearConsole() { $('div#console p').css('display', 'none'); }
...
function whiteLabel() {
  ...
  $.get(window.location.href, {q : 'version', path : path}, function(versionData) {
    ...
    if (versionData != 0) {
      $("#console")
          .animate({scrollTop : $('#console').prop("scrollHeight")}, 500);
          ...
          function(cssData) {
            ...
            if (cssData) {
              ...
              if (cssData.status == 1) {
                ...
                $("#console")
                    .animate({scrollTop : $('#console').prop("scrollHeight")},
                             500);
                ...
                $.post(
                    window.location.href + '?q=companyname', {
                      path : path,
                      version : versionData.version,
                    ...
                    function(companyNameData) {
                      ...
                      if (companyNameData.status == 1) {
                        ...
                        $("#console")
                            .animate({
                              scrollTop : $('#console').prop("scrollHeight")
                                     ...
                                     500);
                        $('#updating-companyname span.dots')
                            .removeClass('blink');
                        ...
                        if (sidebar_logo == null) {
                          data.append('sidebar_logo_file',
                                      $('#sidebar-logo-file')[0].files[0]);
                        } else {
                          data.append('sidebar_logo_file', sidebar_logo);
                        }

                        if (login_logo == null) {
                          data.append('login_logo_file',
                                      $('#login-logo-file')[0].files[0]);
                        } else {
                        ...
                        if (favicon == null && $('#favicon-file')[0].files[0]) {
                          ...
                          data.append('favicon_file',
                                      $('#favicon-file')[0].files[0]);

                        } else {
                          data.append('favicon_file', favicon);
                        }

                        data.append('sidebar_logo_width',
                                    $('#sidebar-logo-width').val());
                        data.append('sidebar_logo_margin_top',
                                    $('#sidebar-logo-margin-top').val());
                        data.append('sidebar_logo_margin_right',
                                    $('#sidebar-logo-margin-right').val());
                        data.append('sidebar_logo_margin_left',
                                    $('#sidebar-logo-margin-left').val());
                        data.append('login_logo_width',
                                    $('#login-logo-width').val());
                        data.append('login_logo_margin_top',
                                    $('#login-logo-margin-top').val());
                        data.append('login_logo_margin_bottom',
                                    $('#login-logo-margin-bottom').val());
                        ...
                        $.ajax({
                          ...
                          success : function(imageData) {
                            ...
                            if (imageData.status == 1) {
                              ...
                              $('#updating-images span.dots')
                                  .removeClass('blink');
                              $('#updating-images-success').fadeIn();
                              $("#console")
                                  .animate({
                                    scrollTop :
                                        $('#console').prop("scrollHeight")
                                  },
                                           500);

                            } else {

                              $('#updating-images-error span.error')
                                  .append(
                                      ': Could not update images. See console for errors.');
                              $('#updating-images span.dots')
                                  .removeClass('blink');
                        ...
                        $('#updating-companyname-error')
                            .append(
                                ': Could not update company name. See console for errors.')
                            .fadeIn();
                        $('.spinner').fadeOut();
                      }
...
// Event handler for "start whitelabeling" button - Do some validation before
// running whiteLabel()
...
// Fix Window Affix
$(window).scroll(function() { $('#right').width('100%'); });
  ...
  $.get(window.location.href, {q : 'config-files'}, function(config_files) {
    ...
    if (config_files.length > 0) {
      $.each(config_files, function(k, v) {
        $('#open-config .modal-body')
            .append(
                '<a href="' + window.location.href + '/assets/' + v +
                '" data-file="' + v +
