if (window.location.href.substr(-1, 1) == "/") {
  var mautic_url = window.location.href.substr(
    0,
...
function updateSidebarLinkHover(color) {
  ...
  $("style#hover").remove();
  $("body").append(
    '<style id="hover">div.sidebar a:hover { color:' +
      sidebar_link_hover +
      " !important; }</style>"
  );
...
function updateDivderLeft(left) {
  ...
  $("body").append(
    '<style id="border-left">.sidebar > li > a::after { left: ' +
      divider_left +
      "px !important; }</style>"
  );
}
...
function updateLoginLogoWidth(width) {
  ...
  $("#login_preview img#login-logo, div.mautic-logo").css(
    "width",
    login_logo_width + "px"
  );
}
...
function loadJsonConfig(config_file = "config.json") {
  ...
  $.get(window.location.href, { q: "saved", file: config_file }, function(
    data
  ) {
    console.log(data);
    ...
    if (data.status == 1) {
      ...
      if (
        ...
        var wl_url = window.location.href.substr(
          0,
          window.location.href.length - 1
        );
      } else {
      ...
      if (data.data.sidebar_logo) {
        ...
        $.get(window.location.href, {
          q: "asset",
          url: encodeURIComponent(wl_url + "/assets/" + data.data.sidebar_logo)
        }).done(function(d) {
          if (d == 1) {
            $("img#mautic-logo").attr(
              "src",
              "assets/" + data.data.sidebar_logo
            );
            $("div#sidebar-logo-upload").hide();
            $("div#sidebar-logo-upload").val(null);
            $("div#sidebar-logo-loaded span").text(
              "assets/" + data.data.sidebar_logo
            );
      ...
      if (data.data.login_logo) {
        $.get(window.location.href, {
          ...
          if (d == 1) {
            ...
            $("div#login-logo-loaded span").text(
              "assets/" + data.data.login_logo
            );
...
// mautic_primary
$("input#mautic-primary").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// mautic_hover
$("input#mautic-hover").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// sidebar_bg
$("input#sidebar-bg").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// sidebar_submenu_bg
$("input#sidebar-submenu-bg").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// sidebar_link
$("input#sidebar-link").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// sidebar_link_hover
$("input#sidebar-link-hover").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// active_icon
$("input#active-icon").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// sidebar_divider
$("input#sidebar-divider").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// submenu_bullet_bg
$("input#submenu-bullet-bg").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
// submenu_bullet_shadow
$("input#submenu-bullet-shadow").on("move.spectrum change.spectrum", function(
  e,
  tinycolor
) {
...
$("body").on(
  "keyup",
  "input#company-name, input#footer-prefix, input#footer",
  function() {
    updateFooterPreview();
  }
);
...
$("input#sidebar-logo-file, input#login-logo-file, input#favicon-file").change(
  function() {
    readImg(this);
...
function saveConfigJson(file = false) {
  ...
  if ($("#favicon-file")[0].files.length == 1) {
    ...
    if (favicon) {
      ...
      if ($("#login-logo-file")[0].files.length == 1) {
        favicon =
          "favicon-" +
          $("#login-logo-file")[0].files[0].name.split(".")[0] +
          ".ico";
      } else {
  ...
  if (
    $("#sidebar-logo-file")[0].files.length > 0 ||
    ...
    if (file != false) {
      ...
      $.post(window.location.href + "?q=save", { config: config }, function(
        data
      ) {
        if (data) {
          $("button i.save-loading").hide();
...
function whiteLabel() {
  ...
  $.get(window.location.href, { q: "version", path: path }, function(
    versionData
  ) {
              ...
              $.post(
                ...
                function(companyNameData) {
                  ...
                  if (companyNameData.status == 1) {
                    ...
                    data.append(
                      "sidebar_logo_margin_top",
                      $("#sidebar-logo-margin-top").val()
                    );
                    data.append(
                      "sidebar_logo_margin_right",
                      $("#sidebar-logo-margin-right").val()
                    );
                    data.append(
                      "sidebar_logo_margin_left",
                    ...
                    data.append(
                      "login_logo_margin_top",
                      $("#login-logo-margin-top").val()
                    );
                    data.append(
                      "login_logo_margin_bottom",
                    ...
                    $.ajax({
                      ...
                      success: function(imageData) {
                        ...
                        if (imageData.status == 1) {
                          ...
                          $("#updating-images-error span.error").append(
