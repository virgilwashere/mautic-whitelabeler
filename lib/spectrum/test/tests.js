...
test("Polyfill", function() {
  ...
  el = $("<input type='color' value='red' />").spectrum({allowEmpty : true});
...
test("Per-element Options Are Read From Data Attributes", function() {
  ...
  equal(el.spectrum("option", "showAlpha"), true,
        "Took showAlpha value from data attribute");

  el.spectrum("destroy");

  var changeDefault =
      $("<input id='spec' data-show-alpha='false' />").spectrum({
        showAlpha : true
      });

  equal(changeDefault.spectrum("option", "showAlpha"), false,
        "Took showAlpha value from data attribute");

  changeDefault.spectrum("destroy");

  var noData = $("<input id='spec' />").spectrum({showAlpha : true});

  equal(noData.spectrum("option", "showAlpha"), true,
...
test("Events Fire", function() {
  ...
  el.on("change",
        function(
            e, color) { ok(false, "Change should not fire from `set` call"); });
  ...
  var el2 = $("<input />").spectrum({showInput : true});
  el2.on("change.spectrum",
         function(e, color) { ok(true, "Change should fire input changing"); });
...
test("Escape hides the colorpicker", function() {
  expect(1);
  var el = $("<input id='spec' />").spectrum();
  el.on("hide.spectrum", function(e) { ok(true, "Hide event should fire"); });
...
test("Default Color Is Set By Input Value", function() {
  ...
  equal(noColor.spectrum("get").toHex(), "000000",
        "Defaults to black with an invalid color");

  var noValue = $("<input id='spec' />").spectrum();
  equal(noValue.spectrum("get").toHex(), "000000",
        "Defaults to black with no value set");

  var noValueHex3 =
      $("<input id='spec' />").spectrum({preferredFormat : "hex3"});
  equal(noValueHex3.spectrum("get").toHex(true), "000",
        "Defaults to 3 char hex with no value set");
  equal(noValueHex3.spectrum("get").toString(), "#000",
        "Defaults to 3 char hex with no value set");
...
test("Palette Events Fire In Correct Order ", function() {
  ...
  el.on("change.spectrum",
        function(e) { ok(count === 1, "change fires after move"); });
...
test("Palette click events work ", function() {
  ...
  el.spectrum("container")
      ...
      .click();
  equal(el.spectrum("get").toName(), "green",
        "Second click worked (on child element)");
  el.spectrum("container")
      .find(".sp-thumb-el:nth-child(1) .sp-thumb-inner")
      .click();
  equal(el.spectrum("get").toName(), "red",
        "Third click worked (on child element)");
  el.spectrum("destroy");
});

test("hideAfterPaletteSelect: Palette stays open after color select",
     function() {
       var el = $("<input id='spec' value='red' />").spectrum({
         showPalette : true,
       ...
       ok(!el.spectrum("container").hasClass('sp-hidden'),
...
test("Local Storage Is Limited ", function() {
  ...
  equal(localStorage["spectrum.tests"],
...
test("allowEmpty", function() {
  var el = $("<input value='red' />").spectrum({allowEmpty : true});
  el.spectrum("set", null);
  ok(!el.spectrum("get"), "input[type] color does not allow null values");
  el.spectrum("destroy");
...
test("clickoutFiresChange", function() {
  var el = $("<input value='red' />").spectrum({clickoutFiresChange : false});
  el.spectrum("show");
  ...
  equal(el.spectrum("get").toName(), "orange", "Color is set");
  $(document).click();
  equal(el.spectrum("get").toName(), "orange",
...
test("replacerClassName", function() {
  var el =
      $("<input />").appendTo("body").spectrum({replacerClassName : "test"});
  ok(el.next(".sp-replacer").hasClass("test"),
...
test("containerClassName", function() {
  var el =
      $("<input />").appendTo("body").spectrum({containerClassName : "test"});
  ok(el.spectrum("container").hasClass("test"),
...
test("Options Can Be Set and Gotten Programmatically", function() {
  ...
  equal(singleOption, "full-spectrum",
  ...
  equal(singleOption, "changed",
        "Changing an option then fetching it is updated");

  var numPaletteElements =
      spec.spectrum("container")
          .find(".sp-palette-row:not(.sp-palette-row-selection) .sp-thumb-el")
          .length;
  ...
  deepEqual(optPalette, [ [ 'red' ], [ 'green' ], [ 'blue' ] ],
            "Changing an option then fetching it is updated");
  numPaletteElements =
      spec.spectrum("container")
          .find(".sp-palette-row:not(.sp-palette-row-selection) .sp-thumb-el")
  ...
  var appendToDefault = $("<input />")
                            .spectrum({

                            });

  var container = $("<div id='c' />").appendTo("body");
  var appendToOther = $("<input />").spectrum({appendTo : container});

  var appendToParent =
      $("<input />").appendTo("#c").spectrum({appendTo : "parent"});

  var appendToOtherFlat =
      $("<input />").spectrum({appendTo : container, flat : true});

  equal(appendToDefault.spectrum("container").parent()[0], document.body,
        "Appended to body by default");

  equal(appendToOther.spectrum("container").parent()[0], container[0],
        "Can be appended to another element");

  equal(appendToOtherFlat.spectrum("container").parent()[0],
        $(appendToOtherFlat).parent()[0],
        "Flat CANNOT be appended to another element, will be same as input");

  equal(appendToParent.spectrum("container").parent()[0], container[0],
        "Passing 'parent' to appendTo works as expected");

  // Issue #70 - https://github.com/bgrins/spectrum/issues/70
  equal(spec.spectrum("option", "showPalette"), true,
        "showPalette is true by default");
  spec.spectrum("option", "showPalette", false);

  equal(spec.spectrum("option", "showPalette"), false,
        "showPalette is false after setting showPalette");
  spec.spectrum("option", "showPaletteOnly", true);

  equal(spec.spectrum("option", "showPaletteOnly"), true,
...
test("Tooltip is formatted based on preferred format", function() {
  ...
  function getTitlesString() {
    return el.spectrum("container")
        .find(".sp-thumb-el")
        .map(function() { return this.getAttribute("title"); })
        .toArray()
        .join(" ");
  }

  equal(getTitlesString(),
        "rgb(255, 0, 0) rgba(255, 255, 255, 0.5) rgb(0, 0, 255)",
        "Titles use rgb format by default");

  el.spectrum("option", "preferredFormat", "hex");
  equal(getTitlesString(), "#ff0000 #ffffff #0000ff",
  ...
  equal(el.spectrum("get").toString(), "hsv(0, 100%, 100%)",
  ...
  equal(el.spectrum("get").toString(), "hsl(0, 100%, 50%)",
        "Value's format is updated");
...
test("Methods work as described", function() {
  ...
  // Method - toggle
  el.spectrum("toggle");
  ok(el.spectrum("container").is(":visible"),
     "Spectrum is visible after toggle");

  el.spectrum("toggle");
  ok(el.spectrum("container").not(":visible"),
  ...
  ok(color.toHsvString() == "hsv(39, 100%, 100%)",
     "Color has been set and gotten as hsv");
  ok(color.toRgbString() == "rgb(255, 165, 0)",
     "Color has been set and gotten as rgb");
  ok(color.toHslString() == "hsl(39, 100%, 50%)",
     "Color has been set and gotten as hsl");

  // Method - container
  ok(el.spectrum("container").hasClass("sp-container"),
  ...
  raises(function() { el.spectrum("no method"); },
...
test(
    ...
    function() {
      ...
      var selectedColor = secondEl.spectrum("container")
                              .find('span[data-color="' + colorToChoose + '"]');
      ok(selectedColor.length > 0,
...
test("Custom offset", function() {
  ...
  var el2 =
      $("<input value='red' />").spectrum({offset : {top : 100, left : 100}});
  el2.spectrum("show");
