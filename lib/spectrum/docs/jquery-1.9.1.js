...
var
    ...
    core_concat = core_deletedIds.concat, core_push = core_deletedIds.push,
    core_slice = core_deletedIds.slice, core_indexOf = core_deletedIds.indexOf,
...
jQuery.fn.extend({
  ...
  addClass : function(value) {
    var classes, elem, cur, clazz, j,
        i = 0, len = this.length, proceed = typeof value === "string" && value;
    ...
    var
        ...
        detail = typeof prop === "boolean"
                     ...
                     getSetInput && getSetAttribute
                         ...
                         // attributes and conflates checked/selected into
                         // attroperties
...
jQuery.event = {
  ...
  dispatch : function(event) {
    ...
    var i, ret, handleObj, matched, j,
        handlerQueue = [], args = core_slice.call(arguments),
...
var i, cachedruns, Expr, getText, isXML, compile, hasDuplicate,
    outermostContext,

    // Local document vars
    setDocument, document, docElem, documentIsXML, rbuggyQSA, rbuggyMatches,
    matches, contains, sortOrder,

    // Instance-specific data
    expando = "sizzle" + -(new Date()), preferredDoc = window.document,
    support = {}, dirruns = 0, done = 0, classCache = createCache(),
    ...
    matchExpr = {
      ...
      "NAME" :
          new RegExp("^\\[name=['\"]?(" + characterEncoding + ")['\"]?\\]"),
...
function addCombinator(matcher, combinator, base) {
  var dir = combinator.dir, checkNonElements = base && dir === "parentNode",
      doneName = done++;
...
function matcherFromGroupMatchers(elementMatchers, setMatchers) {
  ...
  var matcherCachedRuns = 0, bySet = setMatchers.length > 0,
      byElement = elementMatchers.length > 0,
      superMatcher = function(seed, context, xml, results, expandContext) {
        var elem, j, matcher,
            setMatched = [], matchedCount = 0, i = "0", unmatched = seed && [],
            outermost = expandContext != null, contextBackup = outermostContext,
            // We must always have either seed elements or context
            elems = seed ||
                    byElement && Expr.find["TAG"](
...
var runtil = /Until$/, rparentsprev = /^(?:parents|prev(?:Until|All))/,
    isSimple = /^.[^:#\[\.,]*$/, rneedsContext = jQuery.expr.match.needsContext,
  ...
  is : function(selector) {
    return !!selector &&
           (typeof selector === "string"
                ...
                // in the returned set so $("p:first").is("p:last") won't return
...
function winnow(elements, qualifier, keep) {
  ...
  return jQuery.grep(
      elements,
      function(
          elem) { return (jQuery.inArray(elem, qualifier) >= 0) === keep; });
}
...
jQuery.extend({
  ...
  cleanData : function(elems, /* internal */ acceptData) {
    var elem, type, id, data, i = 0, internalKey = jQuery.expando,
                              cache = jQuery.cache,
...
var iframe, getStyles, curCSS,
    ralpha = /alpha\([^)]*\)/i, ropacity = /opacity\s*=\s*([^)]*)/,
...
function ajaxConvert(s, response) {
  var conv2, current, conv, tmp, converters = {}, i = 0,
                                 // Work with a copy of dataTypes in case we
...
var xhrCallbacks, xhrSupported, xhrId = 0,
                                // #5280: Internet Explorer will keep
                                // connections alive if we don't abort on unload
    ...
    tweeners = {
      "*" : [ function(prop, value) {
        var end, unit, tween = this.createTween(prop, value),
                       parts = rfxnum.exec(value), target = tween.cur(),
        ...
        if (parts) {
          ...
          if (unit !== "px" && start) {
            ...
            do {
              ...
              // *something* Use a string for doubling factor so we don't
...
function createTweens(animation, props) {
  jQuery.each(props, function(prop, value) {
    var collection = (tweeners[prop] || []).concat(tweeners["*"]), index = 0,
...
function defaultPrefilter(elem, props, opts) {
  ...
  var prop, index, length, value, dataShow, toggle, tween, hooks, oldfire,
      anim = this, style = elem.style, orig = {}, handled = [],
