 ...
 * @param data
 */
Mautic.loadContent = function(route, link, method, target, showPageLoading,
                              callback, data) {
  ...
  showPageLoading =
      (typeof showPageLoading == 'undefined' || showPageLoading) ? true : false;
 ...
 * @param callback
 */
Mautic.loadAjaxColumn =
    function(elementName, route, callback) {
  ...
  if (mQuery(className).length) {
    ...
    while (ids.length > 0) {
      ...
      Mautic.ajaxActionRequest(
          route, {ids : batchIds, entityId : Mautic.getEntityId()},
          function(response) {
     ...
     * @param sortElement
     */
    Mautic.sortTableByColumn =
        function(tableId, sortElement, removeZero) {
  ...
  tbody.find('tr')
      .sort(function(a, b) {
        var tda = parseFloat(
            mQuery(a).find(sortElement).text()); // target order attribute
        var tdb = parseFloat(
            mQuery(b).find(sortElement).text()); // target order attribute
        // if a < b return 1
        return tda < tdb ? 1
  ...
  if (-1 !== route.indexOf('timeline')) {
    ...
    if (1 < titleWithHTML.length) {
      currentModuleItem =
          titleWithHTML.eq(0).text() + ' - ' + titleWithHTML.eq(1).text();
    } else {
    ...
    mQuery('title').html(currentModule[0].toUpperCase() +
                         currentModule.slice(1) + ' | ' + currentModuleItem +
                         ' | {{company_name}}');
  } else {
    // loading basic title
    mQuery('title').html(mQuery('.page-header h3').html() +
                         ' | {{company_name}}');
  }
};
      ...
      if (response.activeLink) {
        ...
        mQuery(".nav-sidebar").find(".open").each(function() {
          if (!openParent.hasClass('open') ||
              (openParent.hasClass('open') &&
               openParent[0] !== mQuery(this)[0])) {
            mQuery(this).removeClass('open');
          }
        });

        // add current_ancestor classes
        // mQuery(parent).parentsUntil(".nav-sidebar",
        // "li").addClass("current_ancestor");
      }

      mQuery('body').animate({scrollTop : 0}, 0);

    } else {
      var overflow = mQuery(response.target).css('overflow');
      var overflowY = mQuery(response.target).css('overflowY');
      if (overflow == 'auto' || overflow == 'scroll' || overflowY == 'auto' ||
          overflowY == 'scroll') {
        mQuery(response.target).animate({scrollTop : 0}, 0);
      }
    }
...
Mautic.onPageLoad = function(container, response, inModal) {
  Mautic.initDateRangePicker(container + ' #daterange_date_from',
                             container + ' #daterange_date_to');

  // initiate links
  Mautic.makeLinksAlive(mQuery(container + " a[data-toggle='ajax']"));

  // initialize forms
  mQuery(container + " form[data-toggle='ajax']")
      .each(function(index) { Mautic.ajaxifyForm(mQuery(this).attr('name')); });
  ...
  mQuery(container + " *[data-toggle='listfilter']")
      .each(function(index) { Mautic.activateListFilterSelect(mQuery(this)); });
  ...
  pageTooltips.each(function(i) {
    ...
    if (elementParent.get(0).tagName === 'LABEL') {
      ...
      elementParent.hover(function() { thisTooltip.tooltip('show') },
                          function() { thisTooltip.tooltip('hide'); });
    }
  });

  // initialize sortable lists
  mQuery(container + " *[data-toggle='sortablelist']")
      .each(function(index) { Mautic.activateSortable(this); });

  mQuery(container + " div.sortable-panels")
      .each(function() { Mautic.activateSortablePanels(this); });

  // downloads
  mQuery(container + " a[data-toggle='download']").off('click.download');
  mQuery(container + " a[data-toggle='download']")
  ...
  Mautic.makeConfirmationsAlive(
      mQuery(container + " a[data-toggle='confirmation']"));

  // initialize date/time
  mQuery(container + " *[data-toggle='datetime']")
      .each(function() { Mautic.activateDateTimeInputs(this, 'datetime'); });

  mQuery(container + " *[data-toggle='date']")
      .each(function() { Mautic.activateDateTimeInputs(this, 'date'); });

  mQuery(container + " *[data-toggle='time']")
      .each(function() { Mautic.activateDateTimeInputs(this, 'time'); });

  // Initialize callback options
  mQuery(container + " *[data-onload-callback]").each(function() {
    var callback =
        function(el) {
      if (typeof window["Mautic"][mQuery(el).attr('data-onload-callback')] ==
          'function') {
        window["Mautic"][mQuery(el).attr('data-onload-callback')].apply(
            'window', [ el ]);
      }
    }
  ...
  mQuery(container + " input[data-toggle='color']")
      .each(function() { Mautic.activateColorPicker(this); });

  mQuery(container + " select")
      .not('.multiselect, .not-chosen')
      .each(function() { Mautic.activateChosenSelect(this); });

  mQuery(container + " select.multiselect")
      .each(function() { Mautic.activateMultiSelect(this); });

  Mautic.activateLookupTypeahead(mQuery(container));

  // Fix dropdowns in responsive tables -
  // https://github.com/twbs/bootstrap/issues/11037#issuecomment-163746965
  mQuery(container + " .table-responsive").on('shown.bs.dropdown', function(e) {
    var table = mQuery(this), menu = mQuery(e.target).find(".dropdown-menu"),
  ...
  mQuery(container + " .table-responsive")
      .on("hide.bs.dropdown",
          function() { mQuery(this).css("padding-bottom", 0); })
  ...
  mQuery(container + " .nav-overflow-tabs ul")
      .each(function() { Mautic.activateOverflowTabs(this); });

  mQuery(container + " .nav.sortable")
      .each(function() { Mautic.activateSortableTabs(this); });

  // Initialize tab delete buttons
  Mautic.activateTabDeleteButtons(container);

  // spin icons on button click
  mQuery(container + ' .btn:not(.btn-nospin)')
      .on('click.spinningicons',
          function(event) { Mautic.startIconSpinOnEvent(event); });
  ...
  mQuery(container + " .bottom-form-buttons").each(function() {
    if (inModal || mQuery(this).closest('.modal').length) {
        ...
        mQuery(buttons).filter("button").each(function(i, v) {
          ...
          var button = mQuery("<button type='button' />")
                           ...
                           .on('click.ajaxform', function(event) {
                             ...
                             // Disable the form buttons until this action is
                             // complete
                             if (!mQuery(this).hasClass('btn-dnd')) {
                               mQuery(this).parent().find('button').prop(
                                   'disabled', true);
                             }
      ...
      if (mQuery('.toolbar-form-buttons').hasClass('hide')) {
        ...
        mQuery(container +
               ' .toolbar-form-buttons .toolbar-dropdown .drop-menu')
            .html('');
        ...
        mQuery(buttons).filter("button").each(function(i, v) {
          ...
          if (i === lastIndex) {
            ...
            mQuery("<a />")
                ...
                .html(mQuery(this).html())
                .on('click.ajaxform', buttonClick)
                .appendTo(mQuery('<li />').prependTo(
                    '.toolbar-form-buttons .toolbar-dropdown .dropdown-menu'))
          }
        });
  ...
  if (mQuery(container + ' textarea.editor').length) {
    mQuery(container + ' textarea.editor').each(function() {
      ...
      textarea.on('froalaEditor.blur',
                  function(e, editor) { editor.popups.hideAll(); });
      ...
      if (textarea.hasClass('editor-email')) {
        maxButtons = mQuery.grep(
            maxButtons,
            function(value) { return value != 'insertGatedVideo'; });
      ...
      if (textarea.hasClass('editor-advanced') ||
          textarea.hasClass('editor-basic-fullpage')) {
        var options = {
        ...
        textarea.on(
            'froalaEditor.focus',
            function(e, editor) { Mautic.showChangeThemeWarning = true; });

        textarea.froalaEditor(
            mQuery.extend({}, Mautic.basicFroalaOptions, options));
      } else {
  ...
  if (mQuery(container + ' .shuffle-grid').length) {
    ...
    // give a slight delay in order for images to load so that shuffle starts
    // out with correct dimensions
    setTimeout(function() {
      grid.shuffle({itemSelector : ".shuffle", sizer : false});
      ...
      if (grid.parents('.tab-pane').length) {
        ...
        var tab =
            mQuery('a[href="#' + tabId + '"]')
                .on('shown.bs.tab', function() { grid.shuffle("update"); });
      }
    }, 1000);
  }
  ...
  if (container == '#app-content' || container == 'body') {
    ...
    mQuery(".sidebar-left a[data-toggle='ajax']")
        .on('click.ajax',
            function(
                event) { mQuery("html").removeClass('sidebar-open-ltr'); });
    mQuery('.sidebar-right a[data-toggle="ajax"]')
        .on('click.ajax',
            function(
  ...
  if (contentSpecific &&
      typeof Mautic[contentSpecific + "OnLoad"] == 'function') {
    if (inModal ||
        typeof Mautic.loadedContent[contentSpecific] == 'undefined') {
  ...
  if (!inModal && container == 'body') {
    ...
    if (mQuery('#globalSearchContainer').length) {
      mQuery('#globalSearchContainer .search-button').click(function() {
        ...
        setTimeout(function() { mQuery('#globalSearchInput').focus(); }, 100);
        mQuery('body').on('click.globalsearch', function(event) {
          var target = event.target;
          if (!mQuery(target).parents('#globalSearchContainer').length &&
              !mQuery(target).parents('#globalSearchDropdown').length) {
      ...
      Mautic.activateLiveSearch("#globalSearchInput", "lastGlobalSearchStr",
                                "globalLivecache");
    }
  }
  ...
  // stop loading bar
  if ((response && typeof response.stopPageLoading != 'undefined' &&
       response.stopPageLoading) ||
      container == '#app-content' || container == '.page-list') {
    Mautic.stopPageLoadingBar();
  }
};
...
Mautic.activateLookupTypeahead = function(containerEl) {
  containerEl.find("*[data-toggle='field-lookup']").each(function() {
    ...
    Mautic.activateFieldTypeahead(lookup.attr('id'), lookup.attr('data-target'),
...
Mautic.onPageUnload = function(container, response) {
  ...
  if (typeof container != 'undefined') {
    ...
    // unload lingering modals from body so that there will not be multiple
    // modals generated from new ajaxed content
    ...
    mQuery(container + " input[data-toggle='color']")
        .each(function() { mQuery(this).minicolors('destroy'); });
  }
  ...
  if (container == '#app-content') {
    ...
    if (typeof Mautic.chartObjects !== 'undefined') {
      mQuery.each(Mautic.chartObjects, function(i, chart) { chart.destroy(); });
      Mautic.chartObjects = [];
    }

    // trash created map objects to save some memory
    if (typeof Mautic.mapObjects !== 'undefined') {
      mQuery.each(Mautic.mapObjects,
                  function(i, map) { Mautic.destroyMap(map); });
      Mautic.mapObjects = [];
    }
...
Mautic.ajaxifyLink = function(el, event) {
  ...
  if (mQuery(".form-exit-unlock-id").length) {
    if (mQuery(el).attr('data-ignore-formexit') != 'true') {
      var unlockParameter = (mQuery('.form-exit-unlock-parameter').length)
                                ? mQuery('.form-exit-unlock-parameter').val()
                                : '';
      Mautic.unlockEntity(mQuery('.form-exit-unlock-model').val(),
                          mQuery('.form-exit-unlock-id').val(),
                          unlockParameter);
    }
  }
  ...
  if (mQuery('#globalSearchContainer').length &&
      mQuery('#globalSearchContainer').hasClass('active')) {
    Mautic.closeGlobalSearchResults();
  }
...
Mautic.activateChosenSelect = function(el, ignoreGlobal, jQueryVariant) {
  var mQuery =
      (typeof jQueryVariant != 'undefined') ? jQueryVariant : window.mQuery;
  if (mQuery(el).parents('.no-chosen').length && !ignoreGlobal) {
    // Globally ignored chosens because they are handled manually due to hidden
    // elements, etc
    return;
  }
  ...
  if (isLookup) {
    if (mQuery(el).attr('data-new-route')) {
      ...
      mQuery(el).on('change', function() {
        ...
        if (mQuery(el).val() == 'new' &&
            (mQuery(el).attr('data-popup') == "true" ||
             mQuery(el).closest('.modal').length > 0)) {
          ...
          Mautic.loadNewWindow({
            "windowUrl" : url + queryGlue + "contentOnly=1&updateSelect=" +
                              mQuery(el).attr('id')
          });
        } else {
          Mautic.loadAjaxModalBySelectValue(this, 'new', url,
                                            mQuery(el).attr('data-header'));
        }
      });
    }
  ...
  if (typeof mQuery(el).data('chosen-placeholder') !== 'undefined') {
    multiPlaceholder = singlePlaceholder =
        mQuery(el).data('chosen-placeholder');
  }
  ...
  if (isLookup) {
    ...
    if (searchTerm) {
      mQuery(el).ajaxChosen({
        type : 'GET',
        url :
            mauticAjaxUrl + '?action=' + mQuery(el).attr('data-chosen-lookup'),
        dataType : 'json',
        afterTypeDelay : 2,
        minTermLength : 2,
...
Mautic.destroyChosen = function(el) {
  ...
  if (eventObject !== undefined &&
      eventObject['chosen:activate'] !== undefined) {
    el.chosen('destroy');
    el.off('chosen:activate chosen:close chosen:open chosen:updated'); // Clear
                                                                       // chosen
                                                                       // events
                                                                       // because
                                                                       // chosen('destroy')
...
Mautic.activateFieldTypeahead = function(field, target, options, action) {
  if (options && typeof options === 'String') {
    ...
    var fieldTypeahead = Mautic.activateTypeahead(
        '#' + field,
        {dataOptions : values, dataOptionKeys : keys, minLength : 0});
  } else {
    var fieldTypeahead = Mautic.activateTypeahead(
        '#' + field,
        {prefetch : true, remote : true, action : action + "&field=" + target});
  }
  ...
  mQuery(fieldTypeahead)
      .on('typeahead:selected', callback)
      .on('typeahead:autocompleted', callback);
};
...
Mautic.activateMultiSelect = function(el) {
  var moveOption = function(v, prev) {
    ...
    var lastSelected =
        mQuery(el)
            ...
            .filter(function() { return mQuery(this).prop('selected'); })
            .last();

    if (typeof prev !== 'undefined') {
      if (prev) {
        var prevOption =
            mQuery(el).find('option[value="' + prev + '"]').first();
  ...
  mQuery(el).multiSelect({
    afterInit : function(container) {
      ...
      var selectThat = this, $selectableSearch = this.$selectableUl.prev(),
      ...
      if (selectOrder && selectOrder.length > 1) {
        this.deselect_all();
        mQuery.each(selectOrder, function(k, v) { selectThat.select(v); });
      }

      var isSortable = mQuery(el).data('sortable');
      if (isSortable) {
        mQuery(el)
            .parent('.choice-wrapper')
            .find('.ms-selection')
            .first()
            .sortable({
    ...
    selectableHeader :
        "<input type='text' class='ms-search form-control' autocomplete='off'>",
    selectionHeader :
        "<input type='text' class='ms-search form-control' autocomplete='off'>",
    keepOrder : true
  });
...
Mautic.activateModalEmbeddedForms = function(container) {
  mQuery(container + " *[data-embedded-form='cancel']")
      .off('click.embeddedform');
  mQuery(container + " *[data-embedded-form='cancel']")
      .on('click.embeddedform', function(event) {
        event.preventDefault();
        ...
        if (typeof mQuery(this).attr('data-embedded-form-callback') !=
            'undefined') {
          if (typeof window["Mautic"][mQuery(this).attr(
                  'data-embedded-form-callback')] == 'function') {
            window["Mautic"][mQuery(this).attr('data-embedded-form-callback')]
                .apply('window', [ this, modal ]);
          }
        }
      });
  ...
  mQuery(container + " *[data-embedded-form='add']").each(function() {
    ...
    var modal = mQuery(this).closest('.modal');
    if (typeof mQuery(modal).data('bs.modal') !== 'undefined' &&
        typeof mQuery(modal).data('bs.modal').options !== 'undefined') {
    ...
    mQuery(modal).on('show.bs.modal', function() {
      ...
      mQuery(this).on("keydown.embeddedForm", ":input:not(textarea)",
                      function(event) {
                        if (event.keyCode == 13) {
                          event.preventDefault();
...
Mautic.activateSearchAutocomplete = function(elId, modelName) {
  if (mQuery('#' + elId).length) {
    var livesearch =
        (mQuery('#' + elId).attr("data-toggle=['livesearch']")) ? true : false;
...
Mautic.activateLiveSearch = function(el, searchStrVar, liveCacheVar) {
  ...
  mQuery(el).on(
      'focus',
      function() { Mautic.currentSearchString = mQuery(this).val().trim(); });
  mQuery(el).on('change keyup paste', {}, function(event) {
    ...
    if (!enterKeyPressed && Mautic.currentSearchString &&
        Mautic.currentSearchString == searchStr) {
      return;
    }
    ...
    if (overlayEnabled) {
      ...
      mQuery(el).on(
          'blur.livesearchOverlay',
          function() { mQuery(overlayTarget + ' .content-overlay').remove(); });
    }
    ...
    if ((!searchStr && MauticVars[searchStrVar].length) || diff >= 3 ||
        spaceKeyPressed || enterKeyPressed) {
      MauticVars[searchStrVar] = searchStr;
      event.data.livesearch = true;

      Mautic.filterList(event, mQuery(el).attr('id'),
                        mQuery(el).attr('data-action'), target, liveCacheVar,
                        overlayEnabled, overlayTarget);
  ...
  if (mQuery(btn).length) {
    mQuery(btn).on(
        'click', {'parent' : mQuery(el).attr('id')}, function(event) {
          var searchStr = mQuery(el).val().trim();
          MauticVars[searchStrVar] = searchStr;

          Mautic.filterButtonClicked = true;
          Mautic.filterList(event, event.data.parent,
...
Mautic.activateListFilterSelect = function(el) {
  ...
  mQuery(el).on('change', function() {
    ...
    if (prefixExceptions) {
      ...
      if (filterVal.length > 1) {
        for (var i = 0; i < filterVal.length; i++) {
          if (mQuery.inArray(filterVal[i],
                             Mautic.listFilterValues[filterName]) == -1) {
            var newOption =
                mQuery(this).find('option[value="' + filterVal[i] + '"]');
    ...
    var request = window.location.pathname + '?tmpl=' + tmpl + '&' +
                  filterName + '=' + filters;
...
Mautic.activateColorPicker = function(el, options) {
  ...
  if (!pickerOptions) {
    pickerOptions = {
      theme : 'bootstrap',
      change : function(
          hex, opacity) { mQuery(el).trigger('change.minicolors', hex); }
    };
  }
  ...
  if (options.prefetch || options.remote) {
    ...
    var sourceOptions = {
      ...
      dupDetector : function(remoteMatch, localMatch) {
        return (remoteMatch[options.displayKey] ==
                localMatch[options.displayKey]);
      },
      ttl : 15000,
      limit : options.limit
    };
    ...
    var filterClosure = function(list) {
      ...
      if (typeof list == 'object') {
        if (typeof list[0] != 'undefined') {
          ...
          list = mQuery.map(list, function(el) { return el; });
        } else {
    ...
    var lookupOptions = (options.dataOptions) ? options.dataOptions
                                              : mQuery(el).data('options');
      ...
      mQuery(el)
          .typeahead(
              ...
              },
              {
                ...
                source : (typeof theBloodhound != 'undefined')
                             ? theBloodhound.ttAdapter()
                             : substringMatcher(lookupOptions, lookupKeys)
              })
          .on('keypress',
              function(event) {
...
Mautic.activateSortable = function(el) {
  ...
  if (mQuery('#' + prefix + '_additem').length) {
    mQuery('#' + prefix + '_additem').click(function() {
      ...
      mQuery(prototype).appendTo(
          mQuery('#' + prefix + '_list div.list-sortable'));
  ...
  mQuery('#' + prefix + '_list div.list-sortable').sortable({
    ...
    stop : function(i) {
      var order = 0;
      mQuery('#' + prefix + '_list div.list-sortable div.input-group input')
          .each(function() {
            var name = mQuery(this).attr('name');
            if (mQuery(this).hasClass('sortable-label')) {
              name = name.replace(/(\[list\]\[[0-9]+\]\[label\])$/g, '') +
                     '[list][' + order + '][label]';
            } else if (mQuery(this).hasClass('sortable-value')) {
              name = name.replace(/(\[list\]\[[0-9]+\]\[value\])$/g, '') +
                     '[list][' + order + '][value]';
              order++;
            } else {
              name = name.replace(/(\[list\]\[[0-9]+\])$/g, '') + '[list][' +
                     order + ']';
              order++;
            }
...
Mautic.initiateFileDownload = function(link) {
  ...
  var iframe = mQuery("<iframe/>")
                   .attr({src : link, style : "visibility:hidden;display:none"})
