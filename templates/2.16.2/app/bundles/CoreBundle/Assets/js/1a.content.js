...
Mautic.generatePageTitle = function(route) {
  if (-1 !== route.indexOf("timeline")) {
    ...
    // loading basic title
    mQuery("title").html(
      mQuery(".page-header h3").html() + " | {{company_name}}"
    );
  }
};
        ...
        mQuery(".nav-sidebar")
          ...
          .each(function() {
            if (
              ...
              (openParent.hasClass("open") && openParent[0] !== mQuery(this)[0])
            ) {
...
Mautic.onPageLoad = function(container, response, inModal) {
  ...
  mQuery(container + " a[data-toggle='download']").on(
    "click.download",
    function(event) {
  ...
  mQuery(container + " *[data-onload-callback]").each(function() {
    var callback = function(el) {
      if (
        typeof window["Mautic"][mQuery(el).attr("data-onload-callback")] ==
        "function"
      ) {
        window["Mautic"][
          mQuery(el).attr("data-onload-callback")
        ].apply("window", [el]);
      }
    };
  ...
  mQuery(container + " .btn:not(.btn-nospin)").on(
    "click.spinningicons",
    function(event) {
  ...
  mQuery(container + " .bottom-form-buttons").each(function() {
    if (inModal || mQuery(this).closest(".modal").length) {
      ...
      if (mQuery(modal).find(".modal-form-buttons").length) {
        ...
        mQuery(modal)
          .find(".bottom-form-buttons")
          .addClass("hide");
        var buttons = mQuery(modal)
          .find(".bottom-form-buttons")
          .html();
      ...
      if (mQuery(".toolbar-form-buttons").hasClass("hide")) {
        ...
        mQuery(
          container + " .toolbar-form-buttons .toolbar-dropdown .drop-menu"
        ).html("");
    ...
    mQuery(container + " textarea.editor").each(function() {
      ...
      if (textarea.hasClass("editor-builder-tokens")) {
        textarea.on("froalaEditor.initialized", function(e, editor) {
          Mautic.initAtWho(
            editor.$el,
            textarea.attr("data-token-callback"),
            editor
          );
        });
      ...
      if (
        textarea.hasClass("editor-advanced") ||
        ...
        textarea.froalaEditor(
          mQuery.extend({}, Mautic.basicFroalaOptions, {
            // Set custom buttons with separator between them.
    ...
    setTimeout(function() {
      ...
      if (grid.parents(".tab-pane").length) {
        var tabId = grid
          ...
          .first()
          .attr("id");
        var tab = mQuery('a[href="#' + tabId + '"]').on(
          "shown.bs.tab",
          function() {
            grid.shuffle("update");
          }
  ...
  if (container == "#app-content" || container == "body") {
    ...
    mQuery('.sidebar-right a[data-toggle="ajax"]').on("click.ajax", function(
      event
    ) {
    ...
    if (mQuery("#globalSearchContainer").length) {
      mQuery("#globalSearchContainer .search-button").click(function() {
        ...
        mQuery("body").on("click.globalsearch", function(event) {
          ...
          if (
            !mQuery(target).parents("#globalSearchContainer").length &&
  ...
  if (
    (response &&
      typeof response.stopPageLoading != "undefined" &&
...
Mautic.ajaxifyLink = function(el, event) {
  ...
  if (mQuery(".form-exit-unlock-id").length) {
    if (mQuery(el).attr("data-ignore-formexit") != "true") {
      var unlockParameter = mQuery(".form-exit-unlock-parameter").length
        ...
        : "";
      Mautic.unlockEntity(
        mQuery(".form-exit-unlock-model").val(),
  ...
  if (
    mQuery("#globalSearchContainer").length &&
...
Mautic.activateChosenSelect = function(el, ignoreGlobal, jQueryVariant) {
        ...
        if (
          ...
          Mautic.loadNewWindow({
            windowUrl:
              url +
              queryGlue +
              "contentOnly=1&updateSelect=" +
              mQuery(el).attr("id")
          });
        } else {
          Mautic.loadAjaxModalBySelectValue(
            this,
            "new",
            url,
  ...
  if (typeof mQuery(el).data("chosen-placeholder") !== "undefined") {
    multiPlaceholder = singlePlaceholder = mQuery(el).data(
      "chosen-placeholder"
    );
  }
...
Mautic.activateMultiSelect = function(el) {
  var moveOption = function(v, prev) {
    var theOption = mQuery(el)
      .find('option[value="' + v + '"]')
      .first();
    var lastSelected = mQuery(el)
    ...
    if (typeof prev !== "undefined") {
      if (prev) {
        var prevOption = mQuery(el)
          .find('option[value="' + prev + '"]')
          .first();
    ...
    afterInit: function(container) {
      ...
      var selectThat = this,
        ...
        selectionSearchString =
          "#" + this.$container.attr("id") + " .ms-elem-selection.ms-selected";

      this.qs1 = $selectableSearch
...
Mautic.activateModalEmbeddedForms = function(container) {
  mQuery(container + " *[data-embedded-form='cancel']").off(
    "click.embeddedform"
  );
  mQuery(container + " *[data-embedded-form='cancel']").on(
    "click.embeddedform",
    function(event) {
      event.preventDefault();
      ...
      if (
        typeof mQuery(this).attr("data-embedded-form-callback") != "undefined"
      ) {
        if (
          typeof window["Mautic"][
            mQuery(this).attr("data-embedded-form-callback")
          ] == "function"
        ) {
          window["Mautic"][
            mQuery(this).attr("data-embedded-form-callback")
  ...
  mQuery(container + " *[data-embedded-form='add']").each(function() {
    ...
    var modal = mQuery(this).closest(".modal");
    if (
      typeof mQuery(modal).data("bs.modal") !== "undefined" &&
    ...
    mQuery(modal).on("show.bs.modal", function() {
      ...
      mQuery(this).on("keydown.embeddedForm", ":input:not(textarea)", function(
        event
      ) {
        if (event.keyCode == 13) {
          event.preventDefault();
  ...
  mQuery(container + " *[data-embedded-form='add']").on(
    "click.embeddedform",
    function(event) {
      event.preventDefault();
...
Mautic.activateSearchAutocomplete = function(elId, modelName) {
  if (mQuery("#" + elId).length) {
    var livesearch = mQuery("#" + elId).attr("data-toggle=['livesearch']")
      ? true
      : false;
...
Mautic.activateLiveSearch = function(el, searchStrVar, liveCacheVar) {
  ...
  mQuery(el).on("focus", function() {
    Mautic.currentSearchString = mQuery(this)
      .val()
      .trim();
  });
  mQuery(el).on("change keyup paste", {}, function(event) {
    var searchStr = mQuery(el)
      .val()
      .trim();
    ...
    if (prefixExceptions) {
      ...
      if (filterVal.length > 1) {
        for (var i = 0; i < filterVal.length; i++) {
          if (
            mQuery.inArray(filterVal[i], Mautic.listFilterValues[filterName]) ==
            -1
          ) {
            var newOption = mQuery(this).find(
              'option[value="' + filterVal[i] + '"]'
            );
            var prefix = mQuery(newOption)
              .parent()
              .data("prefix");
  ...
  if (options.prefetch || options.remote) {
    ...
    var sourceOptions = {
      ...
      dupDetector: function(remoteMatch, localMatch) {
        return (
          remoteMatch[options.displayKey] == localMatch[options.displayKey]
        );
      },
      ttl: 15000,
      limit: options.limit
    ...
    var lookupOptions = options.dataOptions
      ? options.dataOptions
  ...
  var theTypeahead = mQuery(el)
    .typeahead(
      ...
      },
      {
        ...
        source:
          typeof theBloodhound != "undefined"
